...
def get_app_support_path():
    """Thư mục lưu token Google chuẩn trên macOS (~/Library/Application Support/Auto Drive DESI)
    Đây là vị trí chuẩn, dễ tìm và xóa khi gỡ app"""
...
class AutoDriveDESI(ctk.CTk):
    def __init__(self):
        ...
        # === Biến quản lý chống ngủ (caffeinate) ===
        self.caffeinate_process = None
       
        self.setup_ui()
        self.check_auth_status()
        
        # === TÍNH NĂNG GIỮ MÁY KHÔNG NGỦ KHI ĐANG XỬ LÝ ===
        # Đã thêm caffeinate để Mac không ngủ trong lúc upload/nén/sync đang chạy.
        # Khi xong việc thì Mac được phép ngủ bình thường.
        
        # --- MẸO ÉP MAC RENDER ĐỒ HỌA CHỐNG TRẮNG MÀN ---
    ...
    def toggle_pause(self):
        ...
        if self.is_paused:
            ...
            self.update_ui(self.progress_bar.get() * 100, "⏸️ ĐĐÃ TẠM DỪNG - Nhấn 'Tiếp tục' để chạy tiếp")
    ...
    def cancel_process(self):
        ...
        if response:
            ...
            # === RESET TOÀN BỘ TRẠNG THÁI VỀ 0 (FIX YÊU CẦU) ===
            self.start_time = 0
            self.uploaded_bytes = 0
            self.total_bytes = 0
            self.total_files = 0
            ...
            # Ép reset label tốc độ về trạng thái ban đầu
            self.lbl_speed.configure(text="0 MB/s | ETA: --:-- | File: 0/0")
            self.stop_keep_awake()

    # === TÍNH NĂNG GIỮ MÁY TÍNH KHÔNG NGỦ KHI APP ĐANG LÀM VIỆC ===
    def start_keep_awake(self):
        """Bắt đầu giữ Mac không ngủ (chỉ khi đang xử lý)"""
        if self.caffeinate_process is None:
            try:
                ...
                pass  # Nếu không có caffeinate thì bỏ qua

    def stop_keep_awake(self):
        """Dừng giữ máy không ngủ, cho phép Mac ngủ bình thường"""
    ...
    def logic_process(self):
        self.start_keep_awake()  # Giữ Mac không ngủ trong lúc xử lý
        try:
            mode = self.mode_var.get()
            ...
            if mode == "raw_up":
                ...
                if files and not self.is_cancelled:
                    try:
                        ...
                        messagebox.showerror("Lỗi Upload", f"Không thể upload file 7z lên Drive.\n\nChi tiết: {str(zip_err)}\n\nHãy kiểm tra lại ID thư mục đích hoặc quyền truy cập.", parent=self)
            elif mode == "zip_only":
                self.compress_data()
            ...
            self.stop_keep_awake()  # Cho phép Mac ngủ bình thường sau khi xong
    ...
    def upload_multiple_files_direct(self, file_paths_list):
        ...
        self.start_keep_awake()  # Giữ máy không ngủ khi đang upload (áp dụng cho cả Auto Sync)
        try:
            ...
            self.stop_keep_awake()  # Cho máy ngủ lại sau khi upload xong
    ...
    def download_multiple_files_parallel(self, tasks):
        ...
        self.start_keep_awake()  # Giữ máy không ngủ khi đang download (áp dụng cho Auto Download)
        try:
            ...
            self.stop_keep_awake()  # Cho máy ngủ lại sau khi download xong
    ...
    def compress_data(self):
        ...
        # MAX TỐC ĐỘ NÉN (ưu tiên tốc độ cao nhất - gần như không nén, chỉ đóng gói)
